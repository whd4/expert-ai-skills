...
| 5 | **A deployment must fire an agent that can finish alone.** No `always_ask` tool may be in effect on a scheduled run; `agent_overrides.tools` disables it for the deployment only. The validator cross-checks this. | A deployment has no client attached. The first gated tool call parks the session in `requires_action` - every firing, silently, with the sandbox alive. |
...
- **Never deletes.** An entry the plan no longer wants is unlinked if it is a symlink (the bridge's own artefact) and otherwise moved to `.claude/skills-quarantine/<date>/`. `--no-prune` leaves strays alone. `--dry-run` names exactly what would be created, unlinked, or quarantined.
